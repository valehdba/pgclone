...
-- v2.1.0+v2.1.1: Progress Tracking View with progress bar
...
COMMENT ON FUNCTION pgclone_progress_detail() IS 'Returns tabular progress with visual progress bar for all active/recent clone jobs';

-- VIEW: convenient wrapper
CREATE VIEW pgclone_jobs_view AS
    SELECT * FROM pgclone_progress_detail();

COMMENT ON VIEW pgclone_jobs_view IS 'Live progress tracking view with progress bar for all pgclone async clone jobs';
