...
COMMENT ON FUNCTION pgclone_table(TEXT, TEXT, TEXT, BOOLEAN, TEXT, TEXT) IS 'Clone table with JSON options: {"columns":["col1","col2"], "where":"status=''active''", "indexes":false, "constraints":false, "triggers":false, "mask":{"email":"email","name":"name","phone":"phone","col":{"type":"partial","prefix":2,"suffix":3},"col2":"hash","col3":"null","col4":{"type":"random_int","min":0,"max":100},"col5":{"type":"constant","value":"REDACTED"}}}';
...
COMMENT ON FUNCTION pgclone_progress_detail() IS 'Returns tabular progress with visual progress bar and elapsed time for all clone jobs';

-- VIEW: convenient wrapper
CREATE VIEW pgclone_jobs_view AS
    SELECT * FROM pgclone_progress_detail();

COMMENT ON VIEW pgclone_jobs_view IS 'Live progress tracking view with progress bar and elapsed time for all pgclone async clone jobs';

-- v3.1.0: Auto-discovery of sensitive data
CREATE FUNCTION pgclone_discover_sensitive(source_conninfo TEXT, schema_name TEXT)
RETURNS TEXT AS 'MODULE_PATHNAME', 'pgclone_discover_sensitive'
LANGUAGE C VOLATILE STRICT;
COMMENT ON FUNCTION pgclone_discover_sensitive(TEXT, TEXT) IS 'Scan source schema for columns matching sensitive data patterns (email, name, phone, ssn, salary, etc.) and return suggested mask rules as JSON';

-- v3.2.0: Static data masking on local tables
CREATE FUNCTION pgclone_mask_in_place(schema_name TEXT, table_name TEXT, mask_json TEXT)
RETURNS TEXT AS 'MODULE_PATHNAME', 'pgclone_mask_in_place'
LANGUAGE C VOLATILE STRICT;
COMMENT ON FUNCTION pgclone_mask_in_place(TEXT, TEXT, TEXT) IS 'Apply data masking to an existing local table via UPDATE. mask_json uses same format as clone mask option: {"email": "email", "name": "name", "ssn": "null"}';

-- v3.3.0: Dynamic data masking via views and role-based access
CREATE FUNCTION pgclone_create_masking_policy(schema_name TEXT, table_name TEXT, mask_json TEXT, privileged_role TEXT)
RETURNS TEXT AS 'MODULE_PATHNAME', 'pgclone_create_masking_policy'
LANGUAGE C VOLATILE STRICT;
COMMENT ON FUNCTION pgclone_create_masking_policy(TEXT, TEXT, TEXT, TEXT) IS 'Create a dynamic masking policy: creates a masked view, revokes base table access from PUBLIC, grants view to PUBLIC, grants base table to privileged role';

CREATE FUNCTION pgclone_drop_masking_policy(schema_name TEXT, table_name TEXT)
RETURNS TEXT AS 'MODULE_PATHNAME', 'pgclone_drop_masking_policy'
LANGUAGE C VOLATILE STRICT;
COMMENT ON FUNCTION pgclone_drop_masking_policy(TEXT, TEXT) IS 'Remove a dynamic masking policy: drops the masked view and restores base table access to PUBLIC';

-- v3.4.0: Clone roles with permissions and passwords
CREATE FUNCTION pgclone_clone_roles(source_conninfo TEXT)
RETURNS TEXT AS 'MODULE_PATHNAME', 'pgclone_clone_roles'
LANGUAGE C VOLATILE STRICT;
COMMENT ON FUNCTION pgclone_clone_roles(TEXT) IS 'Clone all non-system roles from source with encrypted passwords, attributes, memberships, and all permissions. Requires superuser on both source and target.';

CREATE FUNCTION pgclone_clone_roles(source_conninfo TEXT, role_names TEXT)
RETURNS TEXT AS 'MODULE_PATHNAME', 'pgclone_clone_roles'
LANGUAGE C VOLATILE;
COMMENT ON FUNCTION pgclone_clone_roles(TEXT, TEXT) IS 'Clone specific roles (comma-separated) from source with encrypted passwords, attributes, memberships, and permissions. If role exists on target, syncs password and attributes without dropping.';

-- v3.5.0: Clone verification — compare row counts
CREATE FUNCTION pgclone_verify(source_conninfo TEXT, schema_name TEXT)
RETURNS TABLE (
    schema_name  TEXT,
    table_name   TEXT,
    source_rows  BIGINT,
...
LANGUAGE C VOLATILE STRICT;
COMMENT ON FUNCTION pgclone_verify(TEXT, TEXT) IS 'Compare row counts between source and local target for all tables in a schema. Returns side-by-side comparison with match status.';

CREATE FUNCTION pgclone_verify(source_conninfo TEXT)
RETURNS TABLE (
    schema_name  TEXT,
    table_name   TEXT,
    source_rows  BIGINT,
...
LANGUAGE C VOLATILE STRICT;
COMMENT ON FUNCTION pgclone_verify(TEXT) IS 'Compare row counts between source and local target for all user tables across all schemas. Returns side-by-side comparison with match status.';

-- v3.6.0: GDPR/Compliance masking report
CREATE FUNCTION pgclone_masking_report(schema_name TEXT)
RETURNS TABLE (
    schema_name    TEXT,
    table_name     TEXT,
    column_name    TEXT,
...
LANGUAGE C VOLATILE STRICT;
COMMENT ON FUNCTION pgclone_masking_report(TEXT) IS 'Generate GDPR/compliance audit report: lists sensitive columns, their masking status, and recommendations. Checks for masked views.';
