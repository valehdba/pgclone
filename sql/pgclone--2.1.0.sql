...
-- NEW in v2.1.0: Progress Tracking View (SRF-backed)
...
COMMENT ON FUNCTION pgclone_progress_detail() IS 'Returns tabular progress for all active/recent clone jobs (requires shared_preload_libraries = pgclone)';

-- VIEW: convenient wrapper
CREATE VIEW pgclone_jobs_view AS
    SELECT * FROM pgclone_progress_detail();

COMMENT ON VIEW pgclone_jobs_view IS 'Live progress tracking view for all pgclone async clone jobs';
