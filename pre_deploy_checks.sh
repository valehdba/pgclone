...
V_C=$(grep -oP 'pgclone \K[0-9]+\.[0-9]+\.[0-9]+' src/pgclone.c 2>/dev/null || echo "NOT_FOUND")
...
V_BADGE=$(grep -oP 'version-\K[0-9]+\.[0-9]+\.[0-9]+' README.md 2>/dev/null | head -1 || echo "NOT_FOUND")
echo "  README badge:    $V_BADGE"

V_TAG_LINK=$(grep -oP 'releases/tag/v\K[0-9]+\.[0-9]+\.[0-9]+' README.md 2>/dev/null | head -1 || echo "NOT_FOUND")
...
MAGIC_COUNT=$(grep -r "PG_MODULE_MAGIC" src/ --include="*.c" | grep -vc "^Binary" || true)
...
INIT_DEFS=$(grep -r "^_PG_init" src/ --include="*.c" | wc -l || true)
...
if grep -rq "d\.adsrc" src/ --include="*.c" --include="*.h" 2>/dev/null; then
    fail "Deprecated d.adsrc found — use pg_get_expr() instead"
else
    pass "No deprecated d.adsrc usage"
fi

MALLOC_HITS=$(grep -rn "\bmalloc\b\|\bcalloc\b\|\brealloc\b" src/ --include="*.c" | grep -v "//\|/\*\|palloc\|pfree\|PQfreemem" || true)
if [[ -z "$MALLOC_HITS" ]]; then
    pass "No raw malloc/calloc/realloc usage (palloc only)"
else
    MALLOC_COUNT=$(echo "$MALLOC_HITS" | wc -l)
    warn "Found $MALLOC_COUNT possible raw malloc/calloc/realloc calls:"
    echo "$MALLOC_HITS" | head -5 | sed 's/^/    /'
fi

FREE_HITS=$(grep -rn "\bfree\b" src/ --include="*.c" | grep -v 'pfree\|PQfreemem\|//\|/\*\| \* \|free_\|context_free\|error-free\|".*free.*"' || true)
if [[ -z "$FREE_HITS" ]]; then
    pass "No raw free() usage (pfree only)"
else
    FREE_COUNT=$(echo "$FREE_HITS" | wc -l)
    warn "Found $FREE_COUNT possible raw free() calls:"
    echo "$FREE_HITS" | head -5 | sed 's/^/    /'
fi

SHADOW_HITS=$(grep -rn "char.*\berrmsg\b\|text.*\berrmsg\b" src/ --include="*.c" | grep -v "ereport\|elog\|errmsg_internal\|/\*\|//" || true)
...
CONNECT_COUNT=$(grep -rc "PQconnectdb\b" src/ --include="*.c" | awk -F: '{s+=$2}END{print s}' || true)
FINISH_COUNT=$(grep -rc "PQfinish\b" src/ --include="*.c" | awk -F: '{s+=$2}END{print s}' || true)
...
EXEC_COUNT=$(grep -rc "PQexec\b\|PQexecParams\b" src/ --include="*.c" | awk -F: '{s+=$2}END{print s}' || true)
CLEAR_COUNT=$(grep -rc "PQclear\b" src/ --include="*.c" | awk -F: '{s+=$2}END{print s}' || true)
...
UNSAFE_STR=$(grep -rn "\bstrcpy\b\|\bstrncpy\b" src/ --include="*.c" --include="*.h" || true)
if [[ -z "$UNSAFE_STR" ]]; then
    pass "No unsafe strcpy/strncpy usage"
else
    STR_COUNT=$(echo "$UNSAFE_STR" | wc -l)
...
if [[ -f "$LATEST_SQL" ]]; then
    C_FUNCS=$(grep "PG_FUNCTION_INFO_V1" src/pgclone.c src/pgclone_bgw.c 2>/dev/null | grep -oP "PG_FUNCTION_INFO_V1\(\K[^)]+" || true)
    MISSING=0
...
TAB_FILES=$(find .github -name "*.yml" -exec grep -Pln "\t" {} \; 2>/dev/null || true)
if [[ -z "$TAB_FILES" ]]; then
...
if [[ -f test/pgclone_test.sql ]]; then
    PLAN=$(grep -oP "SELECT plan\(\K[0-9]+" test/pgclone_test.sql 2>/dev/null || echo "0")
    TEST_COUNT=$(grep -cE "^\s*SELECT\s+(ok|is|isnt|matches|doesnt_match|has_function|has_extension|has_table|has_schema|has_index|has_view|has_column|has_trigger|col_is_pk|col_is_fk|fk_ok|results_eq|lives_ok|throws_ok|cmp_ok|pass)\s*\(" test/pgclone_test.sql 2>/dev/null || echo "0")
...
CONN_LOG=$(grep -rn "elog\s*(LOG\|elog\s*(WARNING\|elog\s*(ERROR\|elog\s*(FATAL" src/ --include="*.c" 2>/dev/null | grep -i "conninfo\|password\|connstr" || true)
if [[ -z "$CONN_LOG" ]]; then
...
if grep -rq "quote_literal_cstr\|quote_identifier" src/ --include="*.c" 2>/dev/null; then
